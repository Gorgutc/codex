...
- [ ] Define the shared runtime contract in `scripts/model-runtime-contract.cjs`, including `MODEL_RUNTIME_TARGET_MS = 120_000`, `MODEL_RUNTIME_PHASE_TIMEOUT_MS = 120_000`, `MODEL_RUNTIME_WATCHDOG_MS = 360_000`, phase-deadline clipping, absolute-deadline handling, general-model planning, context-loss classification, and outcome validation. Derive a module-scope `HEAVIEST_MODEL_CASE` object `{ caseId, publicPath, bytes }` from visible current content plus `fs.statSync`; fail setup if no visible referenced local model exists.
...
- [ ] Implement `runHeaviestModelSmoke(page)` so exact-GLB response/body, normal 3D-tab click, and readiness share one 120-second load phase; every normal Clay/Xray/PBR click, state wait, and snapshot shares one 120-second material phase; and all phases are clipped by one 360-second operational watchdog through the lifecycle snapshot. Require response status 200-299 and verify `.is-on` plus `aria-pressed` after every material click.
...
- [ ] Always emit named `CASE-3d-heaviest-model-ready` and `CASE-3d-heaviest-model-runtime` results with case id, path, bytes, status, and timings. Mark a successful total above 120 seconds with literal `PERF_WARN`; fail any phase above 120 seconds, the whole scenario above 360 seconds, or any non-2xx, missing readiness, wrong state, page error, or unexpected viewer context loss. Record but exclude the intentional `WEBGL_lose_context` capability-probe release by matching its instrumented canvas id.
...
- [ ] Pin the 120-second target, 120-second phase, and 360-second watchdog boundaries; same-model generic fallback; non-2xx; never-ready; wrong-material; page/console error; context-loss; and actual never-resolving phase/watchdog failure modes in `tests/quality/model-runtime-contract.test.mjs`. Keep the fatal-exit fixture self-contained by copying the shared contract beside its temporary verifier.
- [ ] Preserve the 180,008 ms and intermediate 210,000 ms decisions as historical evidence. Record the clean Linux SwiftShader run `31274544296` that superseded the single-ceiling design: ready 74,475 ms, Clay 81,919 ms, Xray 33,010 ms, then total expiry before PBR. Do not add retries or renderer-specific limits.
