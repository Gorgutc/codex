...
- [ ] Define the shared runtime contract in `scripts/model-runtime-contract.cjs`, including `MODEL_RUNTIME_TARGET_MS = 120_000`, `MODEL_RUNTIME_PHASE_TIMEOUT_MS = 180_000`, `MODEL_RUNTIME_WATCHDOG_MS = 600_000`, phase-deadline clipping, absolute-deadline handling, general-model planning, context-loss classification, and outcome validation. Derive a module-scope `HEAVIEST_MODEL_CASE` object `{ caseId, publicPath, bytes }` from visible current content plus `fs.statSync`; fail setup if no visible referenced local model exists.
...
- [ ] Implement `runHeaviestModelSmoke(page)` so exact-GLB response/body, normal 3D-tab click, and readiness share one 180-second load phase; every normal Clay/Xray/PBR click, state wait, and snapshot shares one 180-second material phase; and all phases are clipped by one 600-second operational watchdog through the lifecycle snapshot. Require response status 200-299 and verify `.is-on` plus `aria-pressed` after every material click.
...
- [ ] Always emit named `CASE-3d-heaviest-model-ready` and `CASE-3d-heaviest-model-runtime` results with case id, path, bytes, status, and timings. Mark a successful total above 120 seconds with literal `PERF_WARN`; fail any phase above 180 seconds, the whole scenario above 600 seconds, or any non-2xx, missing readiness, wrong state, page error, or unexpected viewer context loss. Record but exclude the intentional `WEBGL_lose_context` capability-probe release by matching its instrumented canvas id.
- [ ] Open the generic viewer first on the primary page using the smallest visible referenced model. Run nine real pagination remounts through a content-derived adjacent non-heaviest pair, require the expected settled case after every step, and fail closed when no pair exists with two or more visible cases. Close the primary page before running the heaviest-model contract last on a dedicated normal-motion page; close that page after the return to PBR.
...
- [ ] Pin the 120-second target, 180-second phase, and 600-second watchdog boundaries; same-model generic fallback; non-2xx; never-ready; wrong-material; page/console error; context-loss; and actual never-resolving phase/watchdog failure modes in `tests/quality/model-runtime-contract.test.mjs`. Keep the fatal-exit fixture self-contained by copying the shared contract beside its temporary verifier.
- [ ] Preserve the 180,008 ms, intermediate 210,000 ms, and intermediate 120/120/360 split as historical evidence. Record clean Linux SwiftShader run `31274544296`, then exact-head run `31277048918`: attempt 1 completed at 323,940 ms; attempt 2 reached ready at 70,891 ms, logged the exact GLB HTTP 200 and `contextLosses=0`, then hit the Clay 120,000 ms phase deadline at total 208,086 ms. Because that first terminal timeout ended the run, the log does not separately establish the absence of page or console errors. This measured tail supersedes the intermediate split with 120/180/600. Do not add retries or renderer-specific limits, and do not expand again if the active limits fail.
