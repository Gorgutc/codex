...
`test:browser` runs the Original site smoke plus the Design Lab public-mode suite. The latter covers strict opt-in URL parsing, Home/Case navigation across all project IDs, Specimen, Chamber, and Hybrid media lifecycles, mobile behavior, Hybrid Free Assets, i18n, keyboard focus, and axe checks. It then runs `test:motion` on one worker so concurrent browser load cannot contaminate the frame-cadence measurement.

`test:design-lab` is the focused release gate for Specimen, Chamber, the complete opt-in Hybrid Home/Case/Free Assets capability, and the admin preview selector. It runs the same dedicated `test:motion` worker after the functional suite. It runs inside `codex:ship`, so a Design Lab regression cannot pass the mandatory push gate even when the broader deep audit is skipped. Visual baselines remain a separate gate.
...
Use these before major cleanup or release work. `quality:all` includes the deep audit, governance, visual snapshots, Lighthouse, and the Codex ship gate. Lighthouse covers Original Home/Free Assets plus Specimen, Chamber, and Hybrid Home/Case/Free Assets. Formatting is separate to avoid huge mechanical diffs during focused feature fixes. Lighthouse is separate because local Chrome and network conditions can add noise.
