...
- Free Assets: предпросмотр черновика поддержан в F5: настоящий
  `free-assets.html` рендерит черновые тексты, видимость, порядок и base-имена
  медиа; новые pending 3D-превью/постеры, состояние Download и размер архива в
  preview-окне остаются опубликованными и проверяются после публикации.
...
> **Обновлено (FA-POSTER-01).** Слот постера (`thumb` у ассета и
> `category.tagCard.thumb` у обложки категории) больше не ограничен SVG:
> значение — ЛИБО прежнее базовое имя (по-прежнему `./assets/cards/<base>.svg`),
> ЛИБО полный `./assets/...`-путь с собственным расширением
> (`svg`/`png`/`jpg`/`jpeg`/`webp`). Так на карточку кладётся полноценный
> растровый рендер вместо заглушки. Загрузка через админку теперь пишет полный
> путь (`stageMedia` без `valueMode`), `model` остался базовым именем.
> Отсутствие ключа = прежняя конвенция `<id>.svg`, поэтому миграции контента не
> потребовалось и генерация осталась байт-в-байт. Растровый постер ставит
> `data-poster-kind="raster"` на слот (`.fa-card__thumb` в рантайме,
> `.tag-card__thumb` в GEN-регионе и в зеркале превью), и `css/free-assets.css`
> по этому признаку ослабляет фирменную вуаль: у фото снимаются диагональная
> штриховка и primary-заливка, остаётся только лёгкая нижняя подложка. Вектор
> атрибута НЕ получает — его оформление и байты не меняются.
>
> Канон разбора пути — `scripts/fa-poster-path.mjs` (импортируют генератор,
> orphan-аудит и `content-expectations`; `js/free-assets.js`, `admin/js/state.js`
> и `admin/js/preview.js` повторяют тот же обход по сегментам, потому что
> classic script не умеет import). Сегмент: непустой, не `.`/`..`, только
> `[A-Za-z0-9._-]`; путь начинается с `./assets/`; расширение из allowlist; вид
> постера считается по РАЗРЕШЁННОМУ пути. Не-канонические написания (`//`,
> `/./`, косая черта в конце) ОТВЕРГАЮТСЯ, а не нормализуются: раньше такой путь
> проходил валидатор (`path.resolve` его чинил), не рисовался в рантайме и
> попадал в reference-set orphan-аудита сырым — то есть
> `clean-orphan-assets --delete` удалял ЖИВОЙ постер. Ограничение набора
> символов заодно убрало URL-кодирование: генератор и рантайм отдают
> побайтово одинаковый URL. Единственный фолбэк обложки категории —
> `faTagCoverValue` (ключ категории).
...
- Историческая запись итерации H: Free Assets в превью тогда НЕ входил
  (предпросмотр собирал только index.html). Superseded в F5: `admin/js/preview.js`
  теперь собирает настоящий `free-assets.html` с черновыми текстами, видимостью,
  порядком и base-именами медиа; pending медиа и Download-состояние всё ещё
  проверяются после публикации.
