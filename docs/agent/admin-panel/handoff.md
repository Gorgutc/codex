...
- [ ] **Ответить на 3 вопроса кампании** (блокируют части F2/F4/F5):
  хостинг Beget vs Netlify, реальные ZIP-архивы ассетов, индексация
  RU-версии — список в `docs/agent/prod-review/findings.md`,
  раздел «Вопросы владельцу».
...
Следующий шаг: дождаться ответов владельца на 3 вопроса реестра,
затем итерация F1 (`codex/prod-f1-contract`) по находкам потока D
(+ C-02/C-07, E-03, .gitattributes, флак preloader-смоука).
