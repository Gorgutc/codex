...
1. **F-01 (critical, owner)** — прод-хостинг codex.promo — это
   **Beget (nginx/Apache), а не Netlify**: DNS → 45.130.41.162,
   `/.netlify/functions/cms-auth` на проде отдаёт 404, `/admin/`
   перекрыт beget-заглушкой. Значит: OAuth-вход админки на проде не
   работает (только PAT), `netlify.toml` (кэш и будущие
   security-заголовки) на проде инертен. При этом сами файлы сайта
   синхронизированы с репо байт-в-байт (см. F-04) — механизм деплоя
   репо→Beget есть, но в репо не описан. Требуется решение владельца:
   (а) перевести DNS на Netlify — всё работает как спроектировано;
   (б) остаться на Beget — заголовки через `.htaccess`, вход в
   админку только по PAT либо внешний OAuth-прокси.
...
## Вопросы владельцу (блокируют части F2/F4/F5)

1. **F-01**: как сейчас устроен деплой на Beget и что выбираем —
   DNS на Netlify или адаптация под Beget (.htaccess, PAT-only)?
2. **A2-01/E-02**: есть ли реальные ZIP-архивы ассетов? Выложить или
   честно скрывать Download/contentUrl у отсутствующих?
3. **E-01**: индексируем RU-версию (hreflang + ?lang=ru в sitemap)
   или принимаем EN-only индексацию?
