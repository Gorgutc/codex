...
// FA-POSTER-01: poster slot values (item thumb, category tagCard.thumb) resolve
// through the SHARED canonical parser — see scripts/fa-poster-path.mjs for the
// segment rules and for why non-canonical spellings are rejected instead of
// normalized. Because absent/base-name values keep resolving exactly as before,
// no content migration is needed and the generated bytes stay identical until
// the owner actually uploads a raster.
...
// string → base file name without extension. Poster slots are the ONE
// exception and go through checkFaPosterValue below.
...
// FA-POSTER-01: a poster slot accepts the historical plain base name (→ the
// ./assets/cards/<base>.svg default) OR a full './assets/…' path carrying its
// own extension, so a raster render can be published. The shape rules live in
// scripts/fa-poster-path.mjs so the runtime, the admin and the orphan audit
// resolve the SAME path; a non-canonical spelling ('//', '/./', a trailing
// slash) is a violation here rather than something path.resolve quietly fixes
// while the other consumers disagree about the file. The Russian mirror of this
// rule (what the owner sees in the admin panel) lives in admin/js/state.js.
function checkFaPosterValue(violations, where, value) {
  const problem = faPosterProblem(value);
  if (problem !== null) {
    violations.push(`${where}: ${JSON.stringify(value)} ${problem}`);
    return;
  }
  checkAssetFile(violations, where, faPosterPath(value));
}
...
function validateFreeAssets(violations, freeAssets) {
  ...
  for (const category of freeAssets.categories) {
    ...
    // thumb is the cover poster — a base name (./assets/cards/<thumb>.svg) or,
    // since FA-POSTER-01, a full './assets/…' path with its own extension;
    // gameAsset is the optional boolean that emits the "Game" overlay badge.
    // An absent/null thumb falls back to the category key through the SHARED
    // faTagCoverValue (one fallback for the generator, the orphan audit, the
    // admin preview and scripts/content-expectations.mjs) and is validated like
    // any other cover, so a missing default file is still a hard error.
    ...
    if (tagCard === null || typeof tagCard !== 'object' || Array.isArray(tagCard)) {
      ...
      checkFaPosterValue(violations, `${where}: category "${category.key}" tagCard.thumb`, faTagCoverValue(category));
    ...
    for (const item of category.items) {
      ...
      const thumbValue = faEffectiveBase(item, 'thumb');
      if (thumbValue !== null) checkFaPosterValue(violations, `${itemWhere}: thumb`, thumbValue);
...
// poster path needs no special casing here. The banner below documents the
// historical base-name form; the full-path form added by FA-POSTER-01 is
// described in scripts/fa-poster-path.mjs. The banner text is intentionally left
// byte-identical: rewording it would rewrite js/fa-data.js on a catalog that
// has not changed.
 ...
 *   thumb     — the cover poster: a base name (→ ./assets/cards/<thumb>.svg) or,
 *               since FA-POSTER-01, a full './assets/…' path with its own
 *               extension (a raster cover additionally emits
 *               data-poster-kind="raster" on the thumb slot; a vector cover
 *               emits NO attribute and reproduces the historical bytes);
...
function buildFaTagCardsRegion(content) {
  ...
  const cards = visible.map((category, index) => {
    ...
    // FA-POSTER-01: the attribute is emitted ONLY for a raster cover, so an
    // all-SVG catalog reproduces the historical bytes exactly. The kind comes
    // from the RESOLVED path — a base name that merely ends in ".png" resolves
    // to "<name>.png.svg" and is a vector.
    const posterKindAttr = faPosterKind(cover) === 'raster' ? ' data-poster-kind="raster"' : '';
    ...
    const lines = [
      ...
      `          <div class="tag-card__thumb work-card__thumb" data-label="${escapeHtmlAttr(label)}"${posterKindAttr}>`,
      `            <img src="${escapeHtmlAttr(faPosterPath(cover))}" data-i18n-attr="alt:faTag.${camel}.alt" alt="${escapeHtmlAttr(t.alt)}"`,
      imgLine2
    ];
    if (gameAsset) {
      lines.push(
...
function buildFaJsonLdRegion(content) {
  ...
  const items = visibleItems.map(({ item, category }, i) => {
    ...
    // E-05: advertise a thumbnailUrl only when the asset has a real cover. An
    // asset with thumb:null no longer borrows the page OG image as a dishonest
    // thumbnail — it simply carries no thumbnailUrl. FA-POSTER-01: the URL
